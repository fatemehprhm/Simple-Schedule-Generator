...
class ScheduleGenerator:
    ...
    def add_fixed_commitments(self):
        ...
        sleep_time = (wake_time - sleep_duration_mins + 24 * 60) % (24 * 60)
                ...
                if day in self.days:
                    ...
                    if class_info['prep_time'] > 0:
                        ...
                        self.schedule[day].append({
                            ...
                            'start': self.minutes_to_time(start_mins - prep_duration),
    ...
    def add_routine_tasks(self):
        wake_up_mins = self.time_to_minutes(self.user_data['wake_up_time'])
        
        for day in self.days:
            self.schedule[day].append({
                'task': 'Breakfast',
                'start': self.minutes_to_time(wake_up_mins),
                'end': self.minutes_to_time(wake_up_mins + 45),
                'type': 'routine'
            })
            ...
            if self.user_data['cook_dinner']:
                ...
                dinner_start = 18 * 60
                cooking_end = dinner_start + cooking_duration
                self.schedule[day].append({
                    'task': 'Cooking Dinner',
                    'start': self.minutes_to_time(dinner_start),
                    'end': self.minutes_to_time(cooking_end),
                    'type': 'routine'
                })
                self.schedule[day].append({
                    'task': 'Dinner + Relax',
                    'start': self.minutes_to_time(cooking_end),
                    'end': self.minutes_to_time(cooking_end + 60),
                    'type': 'routine'
                })

        self.schedule['Friday'].append({
            'task': 'Weekly Review & Planning',
            'start': '16:30',
            'end': '17:30',
            'type': 'routine'
        })
        self.schedule['Sunday'].append({
            'task': 'Meal Prep',
            'start': '20:30',
            'end': '22:00',
            'type': 'routine'
    ...
    def get_available_slots(self, day):
        ...
        for item in day_schedule:
            ...
            if item_start > last_end_time:
                available_slots.append((last_end_time, item_start))
            last_end_time = max(last_end_time, self.time_to_minutes(item['end']))
        
        if last_end_time < (24 * 60):
            available_slots.append((last_end_time, 24 * 60))
        
        return available_slots

    def schedule_flexible_tasks(self, task_list, task_type):
        scheduled_time = {task['name']: 0 for task in task_list}

        for task in task_list:
            target_weekly_mins = task.get('weekly_hours', 0) * 60
            if target_weekly_mins == 0: continue

            min_session_mins = int(task.get('min_session', 0.5) * 60)
            max_session_mins = int(task.get('max_session', 4) * 60)

            for _ in range(7 * 24): 
                if scheduled_time[task['name']] >= target_weekly_mins:
                    break
                ...
                for day in self.days:
                    ...
                    for start_mins, end_mins in available_slots:
                        ...
                        if slot_duration >= min_session_mins:
                            needed_time = target_weekly_mins - scheduled_time[task['name']]
                            session_length = min(slot_duration, max_session_mins, needed_time)
                            
                            if session_length >= min_session_mins:
                                if best_slot is None or session_length > best_slot[2]:
                                    best_slot = (start_mins, start_mins + session_length, session_length)
                                    best_day = day
                
                if best_slot:
                    start, end, length = best_slot
                    self.schedule[best_day].append({
                        'task': task['name'],
                        'start': self.minutes_to_time(start),
                        'end': self.minutes_to_time(end),
                        'type': task_type
                    })
                    scheduled_time[task['name']] += length
                else:
                    break 

    def schedule_entertainment(self):
        total_entertainment_mins = self.user_data['entertainment_hours'] * 60
        remaining_entertainment_mins = total_entertainment_mins

        while remaining_entertainment_mins > 0:
            best_slot = None
            best_day = None

            for day in self.days:
                for start_mins, end_mins in self.get_available_slots(day):
                    slot_duration = end_mins - start_mins
                    if best_slot is None or slot_duration > best_slot[2]:
                        best_slot = (start_mins, end_mins, slot_duration)
                        best_day = day
            
            if best_slot and best_slot[2] > 15: # Minimum 15-minute slot for entertainment
                start, end, duration = best_slot
                session_length = min(duration, remaining_entertainment_mins, 120) # Max 2-hour entertainment block
                self.schedule[best_day].append({
                    'task': 'Free Time / Entertainment',
                    'start': self.minutes_to_time(start),
                    'end': self.minutes_to_time(start + session_length),
                    'type': 'entertainment'
                })
                remaining_entertainment_mins -= session_length
            else:
                break
    ...
    def print_schedule(self):
        ...
        for day in self.days:
            ...
            for item in day_schedule:
                if item['start'] == '00:00' and item['task'] == 'Sleep':
                    continue
                print(f"{item['start']}-{item['end']} | {item['task']}")

    def save_schedule(self, filename="my_schedule.json"):
    ...
    def generate_schedule(self):
        ...
        self.add_routine_tasks()
        self.schedule_flexible_tasks(self.user_data['learning_goals'], 'learning')
        self.schedule_entertainment()
        
        self.print_schedule()

        total_learning_hours_scheduled = 0
        for day in self.days:
            for item in self.schedule[day]:
                if item['type'] == 'learning':
                    start = self.time_to_minutes(item['start'])
                    end = self.time_to_minutes(item['end'])
                    total_learning_hours_scheduled += (end - start) / 60

        print(f"\n" + "="*50)
        print("SCHEDULE SUMMARY")
        print("="*50)
        print(f"Total requested learning hours per week: {sum(g['weekly_hours'] for g in self.user_data['learning_goals'])}")
        print(f"Total scheduled learning hours: {total_learning_hours_scheduled:.2f}")
